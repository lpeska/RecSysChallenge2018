...
def build_dataset(words, n_words):
  ...
  with open("challenge_album_names.csv", "r") as chTr:
    for tr in chTr:
      tr = tr.replace("\n","")
      if names.get(tr, 0) <= 0:
...
albs = pd.read_csv("albumsCounter.csv", delimiter=";", header=0)
albs.set_index("AlbumID", inplace=True)
albs = albs.Count.to_dict()
albsCount = []  # TODO
for t in range(vocabulary_size):
    albsCount.append(albs.get(reverse_dictionary[t], 1))
print(albsCount[0:30])
...
with graph.as_default():
  ...
  with tf.name_scope('inputs'):
    ...
    albsMultiplier = tf.constant([albsCount], dtype=tf.float32)
  ...
  challengeVals, challengeIndices = tf.nn.top_k(logits,k=100,sorted=True,name=None)
  ...
  albsMultiplier = tf.log(albsMultiplier) + 0.1
  #challenge_sim = tf.multiply(challenge_sim, albsMultiplier)

  challenge_simVals, challenge_simIndices = tf.nn.top_k(challenge_sim,k=100,sorted=True,name=None)
...
with tf.Session(graph=graph) as session:
  ...
  embeddingsFname = "embedImprAlb_word2vec_"+str(vocabulary_size)+"_"+str(embedding_size)+".csv"
  ...
  #with open("challenge_track_predImprAlb_"+str(vocabulary_size)+"_"+str(embedding_size)+".csv","ab") as fPred:
  #  with open("challenge_track_scoreImprAlb_"+str(vocabulary_size)+"_"+str(embedding_size)+".csv", "ab") as fScore:
  with open("challenge_track_predNNAlbFinal_" + str(vocabulary_size) + "_" + str(embedding_size) + ".csv","ab") as knnPred:
            with open("challenge_track_scoreNNAlbFinal_" + str(vocabulary_size) + "_" + str(embedding_size) + ".csv","ab") as knnScore:
