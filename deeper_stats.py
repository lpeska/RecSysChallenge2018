...
def show_summary():
    ...
    for title, count in full_title_histogram.most_common(100):
        print ("%7d %s" % (count, title))
    print()

    print ("top playlist titles")
    for title, count in title_histogram.most_common(100):
        print ("%7d %s" % (count, title))
    print()

    print ("top tracks")
    for track, count in track_histogram.most_common(100):
        print ("%7d %s" % (count, track))

    with open('trackCounter.csv', 'w') as csv_file:
        csv_file.write("TrackID;Count\n")
        for key, value in track_histogram.items():
            csv_file.write(str(key)+";"+str(value)+"\n")

    print()
    print ("top artists")
    for artist, count in artist_histogram.most_common(100):
        print ("%7d %s" % (count, artist))

    with open('artistsCounter.csv', 'w') as csv_file:
        csv_file.write("ArtistID;Count\n")
        for key, value in artist_histogram.items():
            csv_file.write(str(key) + ";" + str(value) + "\n")

    print()
    print ("top albums")
    for artist, count in album_histogram.most_common(100):
        print ("%7d %s" % (count, artist))

    with open('albumsCounter.csv', 'w') as csv_file:
        csv_file.write("AlbumID;Count\n")
        for key, value in album_histogram.items():
            csv_file.write(str(key) + ";" + str(value) + "\n")
